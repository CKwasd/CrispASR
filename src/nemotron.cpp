...
static bool nemotron_run_encoder_chunked(nemotron_context* ctx, const float* pre_enc, int T_enc, int d_model,
    ...
    // general path below.
    if (ctx->backend != ctx->backend_cpu && stream_cache == nullptr && reset_cache && L >= chunk_size) {
...
static bool nemotron_run_preencode(nemotron_context* ctx, const float* mel, int T_mel, std::vector<float>& pre_enc,
    ...
    int T_pre = 0;
    ggml_tensor* pre = nemotron_build_pre_encode(ctx0, mel_t, ctx->model.pre_encode, (int)hp.subsampling_channels,
                                                 &T_pre, ctx->backend != ctx->backend_cpu);
